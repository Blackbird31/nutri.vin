<?php if ($iframe): ?>
    ...
    <div class="input-group shadow-sm">
        ...
        <input type="text" class="form-control text-muted border-bottom-0 rounded-bottom-0" readonly="readonly" value="<?php echo $urlbase."/".$qrcode->id ?>" />
        <button class="input-group-text  border-bottom-0 rounded-bottom-0" type="button"><i class="bi bi-clipboard"></i></button>
        <a class="input-group-text  border-bottom-0 rounded-bottom-0" href="/<?php echo $qrcode->id ?>" target="_blank"><i class="bi bi-box-arrow-up-right"></i></a>
    </div>
<?php else:?>
...
<?php endif; ?>
    ...
    <div class="border border-light-subtle overflow-auto <?php if (!$iframe): ?>rounded-3<?php endif; ?>" style="height: 70vh; width: 390px;" data-liveform-container>
        <?php if ($iframe): ?>
            <iframe src="/<?php echo $qrcode->id ?>" style="height: 99%; width: 388px;"></iframe>
        <?php else: ?>
