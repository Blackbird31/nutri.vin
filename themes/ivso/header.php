<?php include($THEME.'css.php'); ?>

<nav class="navbar navbar-expand-lg bg-body-tertiary">
<div class="container">
  <a class="navbar-brand" href="<?php echo $urlbase; ?>/qrcode/userid/list"><img src="https://declaration.ivsopro.com/images/logo_site_ivso.png" class="mx-auto d-block" alt="Logo nutrivin" height="42"></a>
  <button class="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#navbarNav" aria-controls="navbarNav" aria-expanded="false" aria-label="Toggle navigation">
    <span class="navbar-toggler-icon"></span>
  </button>
  <div class="collapse navbar-collapse" id="navbarNav">
    <ul class="navbar-nav">
      <li class="nav-item">
        <a class="nav-link active" aria-current="page" href="<?php echo $urlbase; ?>/qrcode/userid/list">Mes QRCodes</a>
      </li>
      <li class="nav-item">
        <a class="nav-link" href="<?php echo $urlbase; ?>/disconnect">Déconnexion</a>
      </li>
    </ul>
  </div>
</div>
</nav>
